...
public class ConsultasMunicipio extends Conexion {
    public boolean registrar(Municipio pro) {
        ...
        String sql = "INSERT INTO municipio (nombremunicipio) values (?)";
        try {
    ...
    public boolean modificar(Municipio pro) {
        PreparedStatement ps = null;
        Connection conn = obtenerConexion();
        String sql = "UPDATE municipio SET nombremunicipio=? where idmunicipio=?";
        try {
            ps = conn.prepareStatement(sql);
            ps.setString(1, pro.getNombre());
    ...
    public boolean buscar(Municipio pro) {
        ...
        try {
            ...
            if (rs.next()) {
                pro.setIdmunicipio(rs.getInt("idmunicipio"));
                pro.setNombre(rs.getString("nombremunicipio"));
                return true;
            }
